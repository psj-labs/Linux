...
- 그룹을 생성할 때는 `groupadd` 명령어를 사용합니다. 사용자를 등록하기 전, 해당 사용자가 속할 그룹이 먼저 생성되어 있어야 체계적인 관리가 가능합니다. 형태는 다음과 같습니다.
```text
groupadd [옵션] 그룹명
```
...
- 그룹삭제는는 `groupdel [그룹명]`으로 삭제할 수 있습니다.
...
- 사용자 삭제는 `userdel [옵션] 사용자명`로 할 수 있습니다.
