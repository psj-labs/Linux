...
## 자동 마운트(추가)
- #### 디스크를 부팅 시점에 자동으로 연결해주는 기능입니다.

- `blkid` 명령어는 디스크, 파티션의 UUID, 파일시스템 타입 같은 정보를 확인하는 명령어입니다.

![15](/Linux/File%20System%20&%20Partition/imgs/15.png)

- 필자가 추가한 `sda1`과 `sda5`의 정보도 존재합니다.

- 다음과 같은 경로에 리눅스가 부팅할 때 어떤 저장장치를 어디에, 어떤 방식으로 자동 마운트할지 적어두는 설정 파일이 존재합니다.
```text
/etc/fstab
```
![16](/Linux/File%20System%20&%20Partition/imgs/16.png)

- 이곳에 `sda1`과 `sda5`의 정보를 vi 편집기로 추가하여 부팅될 때마다 자동으로 `sda1`과`sda5`도 마운트 되도록 합니다.

![17](/Linux/File%20System%20&%20Partition/imgs/17.png)

- 재부팅을 하고 `df`로 정보를 확인하면 `sda1`과 `sda5`가 자동으로 마운트 된 것을 확인할 수 있습니다.

![14](/Linux/File%20System%20&%20Partition/imgs/14.png)
